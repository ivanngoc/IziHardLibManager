...
namespace IziHardGames.Projects.Sln
{
    public class SlnProjects
    {
        ...
        public void Add(FileInfo sln, string line)
        {
            SlnProjectRecord record = new SlnProjectRecord(sln, line);
            projects.Add(record);
        }

        public void Add(FileInfo sln, InfoCsproj csproj)
        {
        ...
        public SlnProjectRecord? GetByGuid(Guid guid)
        {
            return projects.FirstOrDefault(x => x.Guid == guid);
        }
        public SlnProjectRecord? GetByGuid(string guid)
        {
            if (string.IsNullOrEmpty(guid)) return null;
            return projects.FirstOrDefault(x => x.Guid == Guid.Parse(guid));
        }
        public SlnProjectRecord? GetByPath(string pathAbs)
        {
            return projects.FirstOrDefault(x => x.PathAbs == pathAbs);
        }

        public void Remove(SlnProjectRecord byGuid)
        {
#if DEBUG
