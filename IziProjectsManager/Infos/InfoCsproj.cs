...
namespace IziHardGames.Projects
{
    public class InfoCsproj : InfoBase
    {
        ...
        internal void SetPairAsmdef(InfoAsmdef item)
        {
            infoAsmdef = item;
            SetPaired(item);
        }

        internal bool TryGetAsmdef(out InfoAsmdef? asmdef)
        {
        ...
        internal Task EnsureRefToUnityCore(JsonObject config)
        {
        ...
        internal static async Task TestAsync3()
        {
        ...
        internal static async Task TestAsync2()
        {
        ...
        internal void AddNested(InfoCsproj infoCsproj)
        {
            nested.Add(infoCsproj);
        }

        internal void FindConnections(List<InfoRelation> result, Func<InfoItem, InfoBase> searcher)
        {
            if (IsNestingSearched) throw new InvalidOperationException($"You must perform nested search! see {nameof(IziProjectsFinding.FindCsprojWithNestedOptionAsync)}()");
            foreach (var nest in this.Nested)
            {
            ...
            if (!IsExecuted) throw new InvalidOperationException($"You must perform nested search! see {nameof(ExecuteAsync)}()");
        ...
        internal void EnsureRefToRelative(InfoCsproj refProj)
        {
        ...
        internal void EnsureRefTo(InfoAsmdef refAsmdef)
        {
            throw new NotImplementedException();
        }
        internal async Task EnsureRefToAsync(InfoAsmdef refAsmdef, InfoCsproj refProj)
        {
        ...
        internal void ForeachProjectReference(Action<ProjectItemElement> value)
        {
            foreach (var item in proj!.Items)
            {
        ...
        internal void ForeachProjectReferenceItem(Action<InfoItem> value)
        {
            foreach (var item in Items)
            {
        ...
        internal static bool IsValidExtension(FileInfo fileInfo)
        {
        ...
        internal static async Task CreateDefault(DirectoryInfo directory, string name)
        {
