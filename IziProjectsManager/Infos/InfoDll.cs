...
namespace IziHardGames.Projects
{
    public class InfoDll : InfoBase
    {
        ...
        internal static async ValueTask<InfoDll?> CreateDefaultAsync(string fullPath)
        {
        ...
        internal bool TryGetByFileName(string name, out DllRecord record)
        {
            return keyValuePairs.Values.TryFindFirst(x => x.filename == name, out record);
        }
        internal void Add(DllRecord record)
        {
