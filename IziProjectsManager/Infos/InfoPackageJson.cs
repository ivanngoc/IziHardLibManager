...
namespace IziHardGames.Projects
{
    public class InfoPackageJson : InfoBase
    {
        ...
        internal void FindConnections(List<InfoRelation> result, Func<string, string, InfoPackageJson> finder)
        {
        ...
        public static async Task CreateNewAsync(DirectoryInfo directory)
        {
            ...
            {
                await CreateDefaultAsync(directory).ConfigureAwait(false);
            }
        }
        ...
        internal static bool IsValid(FileInfo fileInfo)
        {
        ...
        internal static async ValueTask<Guid> GetGuidAsync(FileInfo fileInfo)
        {
        ...
        internal static async Task OverrideGuidAsync(FileInfo fileInfo, Guid toSet)
        {
            string text = await File.ReadAllTextAsync(fileInfo.FullName);
            var jobj = JsonNode.Parse(text)!.AsObject();
        ...
        internal static async Task<bool> IsFromUnityAndMineAsync(FileInfo info)
        {
        ...
        internal static async Task<bool> IsFromUnityAsync(FileInfo info)
        {
            var content = await File.ReadAllTextAsync(info!.FullName);
            // как оказалось в  пакетах Unity asmdef может иметь сразу 2 свойства с одни и тем же именем. Тогда парсер падает. так что если и тут упадет из за этого
            /// то надо использовать <see cref="JsonDocument"/> как в <see cref="InfoAsmdef.ExecuteAsync"/>.FromBytes()
        ...
        internal static async Task<FileInfo?> FindMineAsync(DirectoryInfo dirAsmdef)
        {
