...
namespace IziHardGames.Projects
{
    public class InfoUnityMeta : InfoBase
    {
        ...
        internal void PutRelation(List<InfoRelation> result)
        {
        ...
        internal async Task OverrideGuidInFileAsync(Guid guidStruct)
        {
            SetGuid(guidStruct);
        ...
        internal static bool OverrideGuid(string text, Guid guid, out string newText)
        {
        ...
        internal static async Task SetGuidAsync(FileInfo fileInfo, Guid guid)
        {
            string text = await File.ReadAllTextAsync(fileInfo.FullName).ConfigureAwait(false);
